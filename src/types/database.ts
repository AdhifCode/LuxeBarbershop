/**
 * Database types — manually maintained, mirrors supabase/migrations/0001_init.sql.
 * In production: run `supabase gen types typescript --linked > src/types/database.ts`
 * to regenerate from the live schema.
 *
 * IMPORTANT: Row shapes are declared as `type` (not `interface`) so they are
 * assignable to `Record<string, unknown>`, which the postgrest-js generic
 * `from()` requires. Interfaces are NOT structurally assignable to indexed
 * records in TypeScript's strict mode.
 */
...
// ─── Row types (use `type` not `interface`) ────────
